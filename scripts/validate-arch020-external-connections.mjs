...
const seedBaseline = async () => {
  ...
  await db.$executeRawUnsafe(`INSERT INTO commerce."CommerceToolRevision" (id, "toolId", "revisionNumber", "contractVersion", "createdByAdminId", "definitionVersion", definition) VALUES ('legacy-tool-revision', 'legacy-tool', 1, 'commerce.v1', 'upgrade-admin', '1.0.0', '{"name":"legacy_tool","definitionVersion":"1.0.0","description":"Legacy tool","inputSchema":{},"execution":{},"responseTemplate":{}}'::jsonb)`);
  await db.$executeRawUnsafe(`INSERT INTO commerce."CommerceCapability" (id, key, "displayName", "selectionBinding") VALUES ('legacy-capability', 'conversation_core', 'Conversation Core', 'BASE')`);
  await db.$executeRawUnsafe(`INSERT INTO commerce."CommerceCapabilityRevision" (id, "capabilityId", "revisionNumber", status, "contractVersion", "contentHash", "createdByAdminId", "publishedByAdminId", "publishedAt", "promptTemplate", configuration, "toolBindings") VALUES ('legacy-capability-revision', 'legacy-capability', 1, 'PUBLISHED', 'commerce.v1', repeat('b', 64), 'upgrade-admin', 'upgrade-admin', CURRENT_TIMESTAMP, 'Legacy capability', '{}'::jsonb, '[]'::jsonb)`);
  await db.$executeRawUnsafe(`INSERT INTO commerce."CommerceRelease" (id, "runnerCompatibility", "contractVersion", "responseContract", "responseContractHash", "createdByAdminId") VALUES ('legacy-release', 'runner-v1', 'commerce.v1', '{"version":"response.v1","instructions":"Legacy response","detailsSchema":{}}'::jsonb, repeat('a', 64), 'upgrade-admin')`);
  await db.$executeRawUnsafe(`INSERT INTO commerce."CommerceReleaseCapability" ("releaseId", "capabilityId", "capabilityRevisionId", position) VALUES ('legacy-release', 'legacy-capability', 'legacy-capability-revision', 0)`);
  await db.$executeRawUnsafe(`INSERT INTO commerce."CheckoutRecovery" (id, "shopId", "checkoutToken", "updatedAt", generation, "lastExternalActivityAt") VALUES ('upgrade-recovery', 'upgrade-shop', 'upgrade-checkout-token', CURRENT_TIMESTAMP, 1, CURRENT_TIMESTAMP)`);
  await db.$executeRawUnsafe(`INSERT INTO whatsapp."Conversation" (id, "shopId", "checkoutRecoveryId", type, "inboundVersion", "updatedAt") VALUES ('upgrade-conversation', 'upgrade-shop', 'upgrade-recovery', 'PRODUCT_SUPPORT', 1, CURRENT_TIMESTAMP)`);
  await db.$executeRawUnsafe(`INSERT INTO commerce."CommerceConversationGrant" (id, "shopId", "conversationId", "initialInboundVersion", "releaseId", "selectedCapabilityKeys", "grantedTools", "runnerVersion") VALUES ('legacy-grant', 'upgrade-shop', 'upgrade-conversation', 1, 'legacy-release', '["conversation_core"]'::jsonb, '[]'::jsonb, 'runner-v1')`);
