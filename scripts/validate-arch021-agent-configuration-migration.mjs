...
try {
  ...
  if (mode === 'upgrade') {
    ...
    const indexes=await db.$queryRawUnsafe(`SELECT schemaname,tablename,indexname,indexdef FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog','information_schema') AND tablename <> 'CommerceAuditEvent' AND tablename NOT LIKE 'CommerceModel%' AND tablename NOT LIKE 'CommercePrompt%' AND tablename NOT LIKE 'CommerceAgent%' ORDER BY 1,2,3`);
    migrate('prisma/schema.prisma'); assert.deepEqual(await snapshot(tables),before); const afterIndexes=await db.$queryRawUnsafe(`SELECT schemaname,tablename,indexname,indexdef FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog','information_schema') AND tablename <> 'CommerceAuditEvent' AND tablename NOT LIKE 'CommerceModel%' AND tablename NOT LIKE 'CommercePrompt%' AND tablename NOT LIKE 'CommerceAgent%' ORDER BY 1,2,3`); assert.deepEqual(afterIndexes,indexes); console.log('UPGRADE_PRESERVATION passed');
